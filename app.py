# -*- coding: utf-8 -*-
"""
HidroSed · Módulo Eje Cauce y Secciones v11 · QA hidráulico y geometría común
Aplicación Streamlit independiente para validar eje de cauce, tramo útil, perfil longitudinal,
secciones transversales, QA hidráulico y exportaciones KMZ/Excel/CSV/JSON.
...
@dataclass
class CrossSection:
    # Geometría base de la sección. Las estaciones siempre se interpretan de
    # ribera izquierda a ribera derecha mirando aguas abajo.
    km: float
    center_x: float
    center_y: float
    left_x: float
    ...
    # Modelo geométrico/hidráulico común para integración posterior con HidroSed/HEC-RAS.
    section_id: str = ""
    name: str = ""
    plan: str = "Base"
    section_type: str = "natural"
    topo_source: str = ""
    origin_type: str = "DEM"  # levantada, DEM, curvas, interpolada, modificada
    qa_status: str = "sin_QA"
    ...
    length_downstream_lob_m: Optional[float] = None
    length_downstream_channel_m: Optional[float] = None
    length_downstream_rob_m: Optional[float] = None
    manning_lob: float = 0.045
    manning_channel: float = 0.035
    manning_rob: float = 0.045
    contraction_coeff: float = 0.10
    expansion_coeff: float = 0.30

    ineffective_flow: str = ""
    obstructions: str = ""
    levees_or_limits: str = ""

    original_stations: List[float] = None
    original_elevations: List[float] = None
...
# --------------------------------------------------------------------------------------
# Modelo común de sección, QA y bitácora
# --------------------------------------------------------------------------------------
...
def _append_change(sec: CrossSection, method: str, field: str, old_value: Any, new_value: Any, warnings: Optional[List[str]] = None, plan: Optional[str] = None) -> None:
    """Registra modificaciones sin interferir con el funcionamiento original."""
    import datetime as _dt
    if sec.change_log is None:
        sec.change_log = []
    sec.change_log.append({
        "fecha": _dt.datetime.utcnow().isoformat(timespec="seconds") + "Z",
        "plan": plan or getattr(sec, "plan", "Base"),
        "metodo": method,
        "campo": field,
...
def _estimate_master_points(sec: CrossSection) -> None:
    """Completa talweg y bancos si no fueron definidos por el usuario."""
    stn = _finite_array(sec.stations)
    elev = _finite_array(sec.elevations)
    ...
    order = np.argsort(s)
    s, z = s[order], z[order]
    sec.total_width_m = float(np.nanmax(s) - np.nanmin(s))
    if sec.original_stations is None or sec.original_elevations is None:
        sec.original_stations = [float(v) for v in sec.stations]
        sec.original_elevations = [float(v) if np.isfinite(v) else np.nan for v in sec.elevations]
    if sec.talweg_station is None or sec.talweg_elevation is None:
        i_min = int(np.nanargmin(z))
        sec.talweg_station = float(s[i_min])
        sec.talweg_elevation = float(z[i_min])
    sec.bottom_elevation = float(np.nanmin(z))
    if sec.left_bank_station is None or sec.right_bank_station is None:
        # Criterio conservador: bancos por defecto al 25 % y 75 % del ancho, corregibles en el editor.
        s0, s1 = float(np.nanmin(s)), float(np.nanmax(s))
        width = max(s1 - s0, 1e-6)
        sec.left_bank_station = float(s0 + 0.25 * width)
        sec.right_bank_station = float(s0 + 0.75 * width)
    if sec.left_bank_station is not None and sec.right_bank_station is not None:
        sec.main_channel_width_m = max(0.0, float(sec.right_bank_station) - float(sec.left_bank_station))


def qa_section(sec: CrossSection, axis_useful: Optional[LineString] = None, prev_sec: Optional[CrossSection] = None) -> CrossSection:
    """QA hidráulico geométrico liviano, correctivo cuando es seguro.

    No desarma la lógica existente: corrige duplicados/orden de estaciones y marca advertencias.
    """
    warnings: List[str] = []
    if sec.qa_warnings is None:
        sec.qa_warnings = []
    if not sec.section_id:
        sec.section_id = f"XS_{float(sec.km):010.3f}".replace(".", "_")
    if not sec.name:
        sec.name = f"Sección km {float(sec.km):.3f}"
    if not sec.topo_source:
        sec.topo_source = str(sec.source)
    if not sec.origin_type:
        src = str(sec.source).lower()
        if "interpol" in src:
            sec.origin_type = "interpolada"
        elif "curva" in src:
            sec.origin_type = "curvas"
        elif "excel" in src or "canal" in src:
            sec.origin_type = "modificada"
        else:
            sec.origin_type = "DEM"

    stn = _finite_array(sec.stations)
    elev = _finite_array(sec.elevations)
    if len(stn) != len(elev) or len(stn) < 2:
        warnings.append("Sección con menos de dos puntos station-cota o longitudes inconsistentes.")
    else:
        df = pd.DataFrame({"s": stn, "z": elev}).replace([np.inf, -np.inf], np.nan)
        if df["z"].isna().any():
            warnings.append("La sección contiene cotas nulas/NaN; se omiten en QA.")
        df = df.dropna(subset=["s", "z"]).sort_values("s", kind="mergesort")
        before = len(df)
        df = df.drop_duplicates(subset=["s"], keep="first")
        if len(df) < before:
            warnings.append("Se corrigieron estaciones duplicadas.")
        if len(df) >= 2:
            if not np.all(np.diff(df["s"].to_numpy(dtype=float)) > 0):
                warnings.append("Estaciones transversales no crecientes; se reordenaron.")
            sec.stations = [float(v) for v in df["s"].to_list()]
            sec.elevations = [float(v) for v in df["z"].to_list()]
        else:
            warnings.append("Después de depurar, la sección no conserva puntos suficientes.")

    _estimate_master_points(sec)
    if sec.total_width_m is not None and sec.total_width_m < 1.0:
        warnings.append("Ancho total insuficiente (<1 m).")
    if sec.left_bank_station is not None and sec.right_bank_station is not None:
        if sec.left_bank_station >= sec.right_bank_station:
            warnings.append("Bancos invertidos; se corrigen intercambiando banco izquierdo/derecho.")
            sec.left_bank_station, sec.right_bank_station = sec.right_bank_station, sec.left_bank_station
        if sec.talweg_station is not None and not (sec.left_bank_station <= sec.talweg_station <= sec.right_bank_station):
            warnings.append("Talweg fuera del canal principal definido por bancos.")
    if prev_sec is not None:
        try:
            z0 = np.array(prev_sec.elevations, dtype=float)
            z1 = np.array(sec.elevations, dtype=float)
            if np.isfinite(z0).any() and np.isfinite(z1).any():
                dz = abs(float(np.nanmin(z1)) - float(np.nanmin(z0)))
                dkm = abs(float(sec.km) - float(prev_sec.km))
                if dkm > 0 and dz / max(dkm * 1000.0, 1.0) > 0.20:
                    warnings.append("Cambio abrupto de fondo respecto de la sección vecina.")
        except Exception:
            pass
    sec.qa_warnings = sorted(set((sec.qa_warnings or []) + warnings))
    sec.qa_status = "OK" if not sec.qa_warnings else ("REVISAR" if len(sec.qa_warnings) <= 2 else "CRITICA")
    if sec.qa_status != "OK" and sec.status == "aceptada":
        sec.status = "revisar"
    return sec


def apply_sections_qa(sections: List[CrossSection], axis_useful: Optional[LineString] = None) -> List[CrossSection]:
    sections = sorted(list(sections or []), key=lambda s: float(s.km))
    prev = None
    for i, sec in enumerate(sections):
        if not sec.section_id:
            sec.section_id = f"XS_{i+1:04d}_km_{float(sec.km):.3f}"
        qa_section(sec, axis_useful=axis_useful, prev_sec=prev)
        prev = sec
    # Longitudes hacia la sección aguas abajo por zona LOB/Channel/ROB.
    for i, sec in enumerate(sections):
        if i < len(sections) - 1:
            dl = max(0.0, (float(sections[i+1].km) - float(sec.km)) * 1000.0)
        else:
            dl = 0.0
        sec.length_downstream_lob_m = dl
        sec.length_downstream_channel_m = dl
        sec.length_downstream_rob_m = dl
    return sections


def sections_change_log_df(sections: List[CrossSection]) -> pd.DataFrame:
    rows = []
    for sec in sections or []:
        for ch in sec.change_log or []:
            r = dict(ch)
            r["section_id"] = sec.section_id
            r["km"] = sec.km
            rows.append(r)
    return pd.DataFrame(rows)


def restore_original_section(sec: CrossSection) -> CrossSection:
    if sec.original_stations and sec.original_elevations:
        old = {"stations": sec.stations, "elevations": sec.elevations}
        sec.stations = [float(v) for v in sec.original_stations]
        sec.elevations = [float(v) for v in sec.original_elevations]
        sec.status = "corregida"
        sec.notes = (sec.notes or "") + " | restaurada a geometría original"
        _append_change(sec, "restaurar_original", "station_elevation", old, {"stations": sec.stations, "elevations": sec.elevations})
    return qa_section(sec)


def copy_hydraulic_properties(target: CrossSection, source: CrossSection, method: str) -> CrossSection:
    fields = ["left_bank_station", "right_bank_station", "talweg_station", "manning_lob", "manning_channel", "manning_rob", "contraction_coeff", "expansion_coeff", "ineffective_flow", "obstructions", "levees_or_limits"]
    old = {f: getattr(target, f, None) for f in fields}
    for f in fields:
        setattr(target, f, getattr(source, f, getattr(target, f, None)))
    _append_change(target, method, "propiedades_hidraulicas", old, {f: getattr(target, f, None) for f in fields})
    target.status = "corregida"
    return qa_section(target)


def duplicate_section(sec: CrossSection, delta_km: float = 0.001) -> CrossSection:
    new = _clone_section(sec) if "_clone_section" in globals() else CrossSection(**asdict(sec))
    new.km = float(sec.km) + float(delta_km)
    new.section_id = f"{sec.section_id}_dup" if sec.section_id else f"XS_dup_{new.km:.3f}"
    new.name = f"Duplicada km {new.km:.3f}"
    new.origin_type = "modificada"
    new.status = "corregida"
    new.notes = (new.notes or "") + " | sección duplicada"
    if new.change_log is None:
        new.change_log = []
    _append_change(new, "duplicar_seccion", "km", sec.km, new.km)
    return new


def adaptive_spacing_recommendations(sections: List[CrossSection], axis_useful: Optional[LineString] = None) -> pd.DataFrame:
    """Recomendación liviana de espaciamiento. No recalcula hidráulica; prioriza criticidad geométrica."""
    rows = []
    secs = sorted(sections or [], key=lambda s: float(s.km))
    for a, b in zip(secs[:-1], secs[1:]):
        spacing = (float(b.km) - float(a.km)) * 1000.0
        reasons = []
        crit = "baja"
        try:
            za = np.array(a.elevations, dtype=float)
            zb = np.array(b.elevations, dtype=float)
            wa = max(a.stations) - min(a.stations) if a.stations else np.nan
            wb = max(b.stations) - min(b.stations) if b.stations else np.nan
            if np.isfinite(wa) and np.isfinite(wb) and max(wa, wb) > 0 and abs(wb - wa) / max(wa, wb) > 0.30:
                reasons.append("estrechamiento/ensanchamiento importante")
                crit = "media"
            if np.isfinite(za).any() and np.isfinite(zb).any():
                dz = abs(float(np.nanmin(zb)) - float(np.nanmin(za)))
                if spacing > 0 and dz / spacing > 0.05:
                    reasons.append("cambio importante de pendiente/fondo")
                    crit = "alta"
        except Exception:
            pass
        if spacing > 500:
            reasons.append("separación longitudinal alta")
            crit = "media" if crit == "baja" else crit
        if spacing < 5:
            reasons.append("secciones innecesariamente próximas")
        suggested = spacing
        if crit == "alta":
            suggested = max(25.0, min(spacing / 2.0, 100.0))
        elif crit == "media":
            suggested = max(50.0, min(spacing, 250.0))
        rows.append({
            "km_inicio": float(a.km), "km_fin": float(b.km),
            "espaciamiento_actual_m": float(spacing),
            "espaciamiento_sugerido_m": float(suggested),
            "criticidad": crit,
            "motivo": "; ".join(reasons) if reasons else "sin alerta geométrica",
            "efecto_esperado_densificar": "mejora geométrica/continuidad; requiere corrida hidráulica para cuantificar niveles, velocidad, energía y Froude" if reasons else "bajo",
        })
    return pd.DataFrame(rows)
...
def sections_to_hecras_excel(sections: List[CrossSection]) -> bytes:
    ...
    df_summary = pd.DataFrame([{
        ...
        "right_x": s.right_x,
        "right_y": s.right_y,
        "cota_fondo": s.bottom_elevation,
        "talweg_station": s.talweg_station,
        "banco_izquierdo": s.left_bank_station,
        "banco_derecho": s.right_bank_station,
        "ancho_total": s.total_width_m,
        "ancho_canal_principal": s.main_channel_width_m,
        "longitud_LOB": s.length_downstream_lob_m,
        "longitud_Channel": s.length_downstream_channel_m,
        "longitud_ROB": s.length_downstream_rob_m,
        "Manning_LOB": s.manning_lob,
        "Manning_Channel": s.manning_channel,
        "Manning_ROB": s.manning_rob,
        "coef_contraccion": s.contraction_coeff,
        "coef_expansion": s.expansion_coeff,
        "fuente_topografica": s.topo_source,
        "tipo_seccion": s.section_type,
        "estado_QA": s.qa_status,
        "advertencias_QA": "; ".join(s.qa_warnings or []),
        "origen": s.origin_type,
        "plan": s.plan,
        "source": s.source,
        "status": s.status,
        "notes": s.notes,
    } for s in sections])
    df_changes = sections_change_log_df(sections)
    df_conn = hecras_connection_table(sections)
    with pd.ExcelWriter(mem, engine="openpyxl") as writer:
        df_summary.to_excel(writer, index=False, sheet_name="modelo_geometrico")
        df_points.to_excel(writer, index=False, sheet_name="hecras_station_elevation")
        if not df_conn.empty:
            df_conn.to_excel(writer, index=False, sheet_name="conectividad_longitudinal")
        if not df_changes.empty:
            df_changes.to_excel(writer, index=False, sheet_name="bitacora_cambios")
    mem.seek(0)
    return mem.read()
...
def apply_section_edit(sec: CrossSection, edit_df: pd.DataFrame, status: str, notes: str) -> CrossSection:
    ...
    old = {"stations": list(sec.stations), "elevations": list(sec.elevations), "status": sec.status, "notes": sec.notes}
    sec.stations = [float(v) for v in df["station_m"].to_list()]
    sec.elevations = [float(v) for v in df["cota_m"].to_list()]
    sec.status = status
    sec.notes = notes
    sec.origin_type = "modificada"
    _append_change(sec, "editor_individual", "station_elevation_status_notes", old, {"stations": sec.stations, "elevations": sec.elevations, "status": status, "notes": notes})
    return qa_section(sec)
...
def smooth_section(sec: CrossSection, window: int = 3) -> CrossSection:
    ...
    if len(z) >= 3:
        ...
        _append_change(sec, "suavizar_seccion", "elevations", old, sec.elevations)
    return qa_section(sec)
...
def plot_single_section(sec: CrossSection):
    ...
    if ok.sum() >= 2:
        ...
        # Puntos maestros del modelo común
        if sec.left_bank_station is not None:
            ax.axvline(float(sec.left_bank_station), linestyle=":", linewidth=1.2, alpha=0.9)
            ax.text(float(sec.left_bank_station), base, " BI", va="bottom", ha="left", fontsize=9)
        if sec.right_bank_station is not None:
            ax.axvline(float(sec.right_bank_station), linestyle=":", linewidth=1.2, alpha=0.9)
            ax.text(float(sec.right_bank_station), base, " BD", va="bottom", ha="left", fontsize=9)
        if sec.talweg_station is not None:
            tz = np.interp(float(sec.talweg_station), stn, elev) if len(stn) >= 2 else np.nan
            ax.scatter([float(sec.talweg_station)], [tz], s=65, marker="v", zorder=4)
            ax.text(float(sec.talweg_station), tz, " talweg", va="bottom", ha="left", fontsize=9)
...
def plot_single_section_interactive(sec: CrossSection):
    if go is None:
        return None
    stn = np.array(sec.stations, dtype=float)
    elev = np.array(sec.elevations, dtype=float)
    ...
    if ok.sum() < 2:
        return None
    stn, elev = stn[ok], elev[ok]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=stn, y=elev, mode="lines+markers", name="Sección"))
    ymin, ymax = float(np.nanmin(elev)), float(np.nanmax(elev))
    def add_vline(x, name):
        if x is not None:
            fig.add_trace(go.Scatter(x=[float(x), float(x)], y=[ymin, ymax], mode="lines", name=name, line=dict(dash="dot")))
    add_vline((float(np.nanmin(stn)) + float(np.nanmax(stn))) / 2.0, "Eje")
    add_vline(sec.left_bank_station, "Banco izquierdo")
    add_vline(sec.right_bank_station, "Banco derecho")
    if sec.talweg_station is not None:
        tz = float(np.interp(float(sec.talweg_station), stn, elev))
        fig.add_trace(go.Scatter(x=[float(sec.talweg_station)], y=[tz], mode="markers+text", text=["talweg"], textposition="top right", name="Talweg", marker=dict(size=10, symbol="triangle-down")))
    fig.update_layout(
        title=f"Sección km {sec.km:.3f} · {sec.qa_status}",
        xaxis_title="Station [m] · izquierda → derecha mirando aguas abajo",
        yaxis_title="Cota [m]",
        height=470,
    )
    return fig
...
def _clean_station_elevation(sec: CrossSection) -> Tuple[np.ndarray, np.ndarray]:
    """Estación-cota depurada, ordenada y sin duplicados."""
    stn = np.array(sec.stations, dtype=float)
    z = np.array(sec.elevations, dtype=float)
    ok = np.isfinite(stn) & np.isfinite(z)
    stn, z = stn[ok], z[ok]
    if len(stn) < 2:
        return np.array([]), np.array([])
    order = np.argsort(stn)
    stn, z = stn[order], z[order]
    df = pd.DataFrame({"s": stn, "z": z}).drop_duplicates(subset="s", keep="first")
    return df["s"].to_numpy(dtype=float), df["z"].to_numpy(dtype=float)


def _hecras_master_stations(sec: CrossSection) -> Optional[Dict[str, float]]:
    """Puntos maestros para conectar secciones como una geometría 1D tipo HEC-RAS.

    HEC-RAS no une secciones por coordenadas UTM arbitrarias, sino por zonas hidráulicas
    homologables: sobrebanco izquierdo, canal y sobrebanco derecho. Por eso se usan
    extremo izquierdo, banco izquierdo, talweg, banco derecho y extremo derecho.
    """
    stn, z = _clean_station_elevation(sec)
    if len(stn) < 2:
        return None
    s0, s1 = float(stn[0]), float(stn[-1])
    width = max(s1 - s0, 1e-6)
    lb = sec.left_bank_station if sec.left_bank_station is not None else s0 + 0.25 * width
    rb = sec.right_bank_station if sec.right_bank_station is not None else s0 + 0.75 * width
    tw = sec.talweg_station if sec.talweg_station is not None else float(stn[int(np.nanargmin(z))])
    lb = float(np.clip(float(lb), s0, s1))
    rb = float(np.clip(float(rb), s0, s1))
    if lb >= rb:
        lb, rb = min(lb, rb), max(lb, rb)
        if abs(rb - lb) < 1e-6:
            lb, rb = s0 + 0.25 * width, s0 + 0.75 * width
    tw = float(np.clip(float(tw), lb, rb))
    center = (s0 + s1) / 2.0
    return {"left": s0, "bank_left": lb, "talweg": tw, "bank_right": rb, "right": s1, "center": center}


def _z_at_station(sec: CrossSection, station: float) -> float:
    stn, z = _clean_station_elevation(sec)
    if len(stn) < 2:
        return float("nan")
    return float(np.interp(float(station), stn, z))


def _sample_zone_for_hecras_connection(sec: CrossSection, zone: str, npts: int) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    """Muestrea una zona hidráulica homologable de la sección."""
    m = _hecras_master_stations(sec)
    if m is None:
        return None
    if zone == "LOB":
        a, b = m["left"], m["bank_left"]
    elif zone == "Channel":
        a, b = m["bank_left"], m["bank_right"]
    elif zone == "ROB":
        a, b = m["bank_right"], m["right"]
    else:
        return None
    if abs(float(b) - float(a)) < 1e-6:
        return None
    stations = np.linspace(float(a), float(b), max(2, int(npts)))
    offsets = stations - float(m["center"])
    zz = np.array([_z_at_station(sec, s) for s in stations], dtype=float)
    return offsets, zz


def _filter_sections_for_connection(sections: List[CrossSection], include_rejected: bool = False, max_sections: int = 350) -> List[CrossSection]:
    secs = [s for s in sorted(sections or [], key=lambda q: float(q.km)) if include_rejected or str(s.status).lower() != "rechazada"]
    # Evita sobrecargar Plotly/Streamlit si hay muchas secciones; conserva extremos y muestrea intermedias.
    if len(secs) > max_sections:
        idx = np.linspace(0, len(secs) - 1, max_sections).round().astype(int)
        secs = [secs[i] for i in sorted(set(idx.tolist()))]
    return secs


def build_hecras_longitudinal_mesh(sections: List[CrossSection], include_rejected: bool = False) -> Dict[str, Any]:
    """Construye superficies longitudinales tipo HEC-RAS.

    Devuelve tres superficies: LOB, Channel y ROB. Cada superficie conecta puntos
    equivalentes de secciones consecutivas mediante coordenada transversal normalizada.
    """
    secs = _filter_sections_for_connection(sections, include_rejected=include_rejected)
    secs = [s for s in secs if _hecras_master_stations(s) is not None]
    if len(secs) < 2:
        return {"sections": secs, "zones": {}, "masters": {}, "warnings": ["Se requieren al menos dos secciones válidas para crear conexión longitudinal."]}
    zone_specs = {"LOB": 7, "Channel": 17, "ROB": 7}
    zones: Dict[str, Dict[str, np.ndarray]] = {}
    warnings: List[str] = []
    for zone, npts in zone_specs.items():
        X, Y, Z = [], [], []
        used = 0
        for sec in secs:
            sampled = _sample_zone_for_hecras_connection(sec, zone, npts)
            if sampled is None:
                X.append([float(sec.km) * 1000.0] * npts)
                Y.append([np.nan] * npts)
                Z.append([np.nan] * npts)
                continue
            off, zz = sampled
            X.append([float(sec.km) * 1000.0] * len(off))
            Y.append(off.tolist())
            Z.append(zz.tolist())
            used += 1
        if used >= 2:
            zones[zone] = {"x": np.array(X, dtype=float), "y": np.array(Y, dtype=float), "z": np.array(Z, dtype=float)}
        else:
            warnings.append(f"Zona {zone} sin datos suficientes para conectar.")
    masters: Dict[str, Dict[str, List[float]]] = {}
    for key in ["left", "bank_left", "talweg", "bank_right", "right"]:
        xs, ys, zs = [], [], []
        for sec in secs:
            m = _hecras_master_stations(sec)
            if m is None:
                continue
            station = float(m[key])
            xs.append(float(sec.km) * 1000.0)
            ys.append(station - float(m["center"]))
            zs.append(_z_at_station(sec, station))
        if len(xs) >= 2:
            masters[key] = {"x": xs, "y": ys, "z": zs}
    return {"sections": secs, "zones": zones, "masters": masters, "warnings": warnings}


def hecras_connection_table(sections: List[CrossSection]) -> pd.DataFrame:
    """Tabla de conectividad longitudinal entre secciones consecutivas."""
    secs = sorted(sections or [], key=lambda s: float(s.km))
    rows = []
    for i, (a, b) in enumerate(zip(secs[:-1], secs[1:]), start=1):
        ma = _hecras_master_stations(a)
        mb = _hecras_master_stations(b)
        if ma is None or mb is None:
            continue
        za_tw = _z_at_station(a, ma["talweg"])
        zb_tw = _z_at_station(b, mb["talweg"])
        dx = max(0.0, (float(b.km) - float(a.km)) * 1000.0)
        rows.append({
            "tramo_id": i,
            "section_upstream_id": a.section_id,
            "section_downstream_id": b.section_id,
            "km_upstream": float(a.km),
            "km_downstream": float(b.km),
            "longitud_channel_m": float(a.length_downstream_channel_m if a.length_downstream_channel_m is not None else dx),
            "longitud_LOB_m": float(a.length_downstream_lob_m if a.length_downstream_lob_m is not None else dx),
            "longitud_ROB_m": float(a.length_downstream_rob_m if a.length_downstream_rob_m is not None else dx),
            "cota_talweg_upstream": za_tw,
            "cota_talweg_downstream": zb_tw,
            "pendiente_talweg_m_m": (za_tw - zb_tw) / dx if dx > 0 and np.isfinite(za_tw) and np.isfinite(zb_tw) else np.nan,
            "ancho_upstream_m": float((ma["right"] - ma["left"])),
            "ancho_downstream_m": float((mb["right"] - mb["left"])),
            "ancho_canal_upstream_m": float((ma["bank_right"] - ma["bank_left"])),
            "ancho_canal_downstream_m": float((mb["bank_right"] - mb["bank_left"])),
            "estado_conectividad": "OK" if dx > 0 else "REVISAR",
        })
    return pd.DataFrame(rows)


def plot_sections_3d_interactive(axis_useful: LineString, profile_df: pd.DataFrame, sections: List[CrossSection], connected_mesh: bool = True, include_rejected_mesh: bool = False):
    ...
    if connected_mesh:
        mesh = build_hecras_longitudinal_mesh(sections, include_rejected=include_rejected_mesh)
        zone_labels = {"LOB": "Sobrebanco izquierdo", "Channel": "Canal principal", "ROB": "Sobrebanco derecho"}
        for zone_name, data in mesh.get("zones", {}).items():
            fig.add_trace(go.Surface(
                x=data["x"], y=data["y"], z=data["z"],
                name=zone_labels.get(zone_name, zone_name),
                showscale=False,
                opacity=0.42,
                hovertemplate=(zone_labels.get(zone_name, zone_name) +
                               "<br>Progresiva=%{x:.2f} m<br>Offset=%{y:.2f} m<br>Cota=%{z:.2f} m<extra></extra>"),
        ...
        master_names = {
            ...
            "bank_left": "Banco izq.",
            "talweg": "Talweg",
            "bank_right": "Banco der.",
            "right": "Extremo der.",
        }
        for key, data in mesh.get("masters", {}).items():
            fig.add_trace(go.Scatter3d(
                x=data["x"], y=data["y"], z=data["z"],
                mode="lines", name=master_names.get(key, key),
                line=dict(width=4, dash="dash" if key in ["bank_left", "bank_right"] else "solid"),
                hovertemplate=(master_names.get(key, key) + "<br>Progresiva=%{x:.2f} m<br>Offset=%{y:.2f} m<br>Cota=%{z:.2f} m<extra></extra>"),
            ))
    ...
    fig.update_layout(
        title="Modelo 3D hidráulico conectado tipo HEC-RAS: X=progresiva, Y=offset, Z=cota",
        scene=dict(
...
def apply_batch_section_improvement(sections: List[CrossSection], km_from: float, km_to: float,
    ...
    for i, sec in enumerate(sections):
        if a <= float(sec.km) <= b:
            old = {"stations": list(sec.stations), "elevations": list(sec.elevations), "status": sec.status}
            ...
            _append_change(sec, "mejora_por_tramo", f"km {a:.3f}-{b:.3f}", old, {"stations": sec.stations, "elevations": sec.elevations, "status": sec.status})
            sections[i] = qa_section(sec)
            count += 1
...
def _clone_section(sec: CrossSection) -> CrossSection:
    data = asdict(sec)
    return CrossSection(**copy.deepcopy(data))
...
def _interp_elev_on_section(sec: CrossSection, station: float) -> float:
    st = np.array(sec.stations, dtype=float)
    z = np.array(sec.elevations, dtype=float)
    ok = np.isfinite(st) & np.isfinite(z)
    if ok.sum() < 2:
        return float(np.nanmin(z)) if np.isfinite(z).any() else 0.0
    st, z = st[ok], z[ok]
    order = np.argsort(st)
    st, z = st[order], z[order]
    return float(np.interp(float(station), st, z, left=z[0], right=z[-1]))


def _section_anchor_points(sec: CrossSection) -> Tuple[np.ndarray, np.ndarray, Dict[str, float]]:
    """Puntos maestros homologables para interpolación controlada.

    Incluye extremo izquierdo, banco izquierdo, talweg, banco derecho, extremo derecho
    y puntos adicionales en quiebres geométricos relevantes.
    """
    qa_section(sec)
    st = np.array(sec.stations, dtype=float)
    z = np.array(sec.elevations, dtype=float)
    ...
    st, z = st[ok], z[ok]
    if len(st) < 2:
        raise ValueError("Sección sin puntos suficientes para interpolación avanzada.")
    order = np.argsort(st)
    st, z = st[order], z[order]
    s0, s1 = float(st[0]), float(st[-1])
    width = max(s1 - s0, 1e-9)
    masters = [s0, s1]
    for val in [sec.left_bank_station, sec.talweg_station, sec.right_bank_station]:
        if val is not None:
            masters.append(float(np.clip(val, s0, s1)))
    # Puntos adicionales donde hay cambios relevantes de pendiente transversal.
    if len(st) >= 5:
        dz = np.gradient(z, st)
        ddz = np.abs(np.gradient(dz, st))
        if np.isfinite(ddz).any():
            thr = np.nanpercentile(ddz, 80)
            for s_extra in st[ddz >= thr][:6]:
                masters.append(float(s_extra))
    # Agregar cuantiles para evitar zonas demasiado pobres.
    masters.extend([s0 + 0.25 * width, s0 + 0.50 * width, s0 + 0.75 * width])
    masters = sorted(set(round(float(m), 6) for m in masters if s0 - 1e-6 <= float(m) <= s1 + 1e-6))
    z_m = np.array([_interp_elev_on_section(sec, m) for m in masters], dtype=float)
    t_m = (np.array(masters, dtype=float) - s0) / width
    props = {
        "width": width,
        "left_bank_t": (float(sec.left_bank_station) - s0) / width if sec.left_bank_station is not None else 0.25,
        "talweg_t": (float(sec.talweg_station) - s0) / width if sec.talweg_station is not None else float(t_m[int(np.nanargmin(z_m))]),
        "right_bank_t": (float(sec.right_bank_station) - s0) / width if sec.right_bank_station is not None else 0.75,
    }
    return t_m.astype(float), z_m.astype(float), props
...
def _interpolate_section_at_km_from_brackets(axis_useful: LineString,
    ...
    """Interpolación geométrica controlada entre secciones ancla válidas.

    Usa puntos maestros homologables: extremo izquierdo, banco izquierdo, talweg, banco derecho,
    extremo derecho y quiebres geométricos. La sección resultante se marca siempre como
    interpolada, nunca como levantamiento real.
    """
    refs = sorted([s for s in ref_sections if s.stations and s.elevations], key=lambda s: float(s.km))
    refs = apply_sections_qa(refs, axis_useful)
    if len(refs) < 2:
        return None
    km = float(target_km)
    prev_sec = None
    ...
    if abs(k1 - k0) < 1e-9:
        ...
        w = float(np.clip((km - k0) / max(k1 - k0, 1e-9), 0.0, 1.0))

    try:
        t0, z0, p0 = _section_anchor_points(prev_sec)
        t1, z1, p1 = _section_anchor_points(next_sec)
    except Exception:
        try:
            t0, z0, hw0 = _section_profile_normalized(prev_sec)
            t1, z1, hw1 = _section_profile_normalized(next_sec)
            p0, p1 = {"width": 2*hw0, "left_bank_t":0.25, "talweg_t":0.5, "right_bank_t":0.75}, {"width": 2*hw1, "left_bank_t":0.25, "talweg_t":0.5, "right_bank_t":0.75}
        except Exception:
            return None

    width = float(fixed_half_width_m) * 2.0 if fixed_half_width_m and fixed_half_width_m > 0 else (1.0 - w) * p0["width"] + w * p1["width"]
    width = max(width, 2.0)
    step = max(float(station_step_m or 5.0), 0.5)
    n = max(2, int(math.ceil(width / step)) + 1)
    stations = np.linspace(0.0, width, n)
    tn = stations / max(width, 1e-9)
    ...
    sec_line = cross_section_line(axis_useful, d, width / 2.0)
    ...
    sec = CrossSection(
        ...
        source="Interpolación avanzada entre secciones ancla",
        status="corregida",
        origin_type="interpolada",
        section_type="interpolada",
        plan=getattr(prev_sec, "plan", "Base"),
        left_bank_station=float(((1.0 - w) * p0["left_bank_t"] + w * p1["left_bank_t"]) * width),
        talweg_station=float(((1.0 - w) * p0["talweg_t"] + w * p1["talweg_t"]) * width),
        right_bank_station=float(((1.0 - w) * p0["right_bank_t"] + w * p1["right_bank_t"]) * width),
        manning_lob=float((1.0 - w) * prev_sec.manning_lob + w * next_sec.manning_lob),
        manning_channel=float((1.0 - w) * prev_sec.manning_channel + w * next_sec.manning_channel),
        manning_rob=float((1.0 - w) * prev_sec.manning_rob + w * next_sec.manning_rob),
        contraction_coeff=float((1.0 - w) * prev_sec.contraction_coeff + w * next_sec.contraction_coeff),
        expansion_coeff=float((1.0 - w) * prev_sec.expansion_coeff + w * next_sec.expansion_coeff),
        notes=f"Interpolada entre secciones ancla km {k0:.3f} y km {k1:.3f}; no corresponde a levantamiento real.",
    )
    _append_change(sec, "interpolacion_avanzada", "seccion_completa", f"ancla {k0:.3f}/{k1:.3f}", f"km {km:.3f}")
    return qa_section(sec, axis_useful=axis_useful)
...
def apply_prismatic_channel_by_km(sections: List[CrossSection], axis_useful: LineString,
    ...
    for i, sec in enumerate(sections):
        ...
        sec.right_x = sec_line.coords[-1][0]
        sec.right_y = sec_line.coords[-1][1]
        old_geom = {"stations": list(sec.stations), "elevations": list(sec.elevations), "source": sec.source}
        sec.stations = [float(v) for v in stations]
        sec.elevations = [float(v) for v in elevations]
        sec.source = source
        sec.section_type = kind_l
        sec.origin_type = "modificada"
        sec.manning_lob = float(manning_n)
        sec.manning_channel = float(manning_n)
        sec.manning_rob = float(manning_n)
        sec.status = "corregida"
        # Corte/relleno aproximado por diferencia station-elevation, útil solo como control geométrico preliminar.
        try:
            old_s = np.array(old_geom["stations"], dtype=float)
            old_z = np.array(old_geom["elevations"], dtype=float)
            new_s = np.array(sec.stations, dtype=float)
            new_z = np.array(sec.elevations, dtype=float)
            grid = np.linspace(max(np.nanmin(old_s), np.nanmin(new_s)), min(np.nanmax(old_s), np.nanmax(new_s)), 80)
            dz = np.interp(grid, new_s, new_z) - np.interp(grid, old_s, old_z)
            cut_area = float(np.trapz(np.maximum(-dz, 0.0), grid))
            fill_area = float(np.trapz(np.maximum(dz, 0.0), grid))
            cf_txt = f" corte_aprox={cut_area:.2f} m2; relleno_aprox={fill_area:.2f} m2."
        except Exception:
            cf_txt = ""
        sec.notes = (sec.notes or "") + f" | reemplazada por {source} entre km {a:.3f}-{b:.3f}.{cf_txt} {note}".strip()
        _append_change(sec, "modificacion_cauce_prismatico", "geometria", old_geom, {"stations": sec.stations, "elevations": sec.elevations, "source": source})
        sections[i] = qa_section(sec, axis_useful=axis_useful)
        count += 1
    return sections, count


# --------------------------------------------------------------------------------------
# Inserción de secciones definidas por puntos station-cota-coordenadas
# --------------------------------------------------------------------------------------
def section_points_template_excel() -> bytes:
    """Plantilla para insertar secciones por puntos.

    Columnas mínimas: station_m, cota_m. Recomendadas: km, x, y.
    Si km queda vacío, la geometría se replica entre km inicial y km final con la separación definida.
    """
    df = pd.DataFrame([
        {"km": None, "station_m": -20.0, "x": None, "y": None, "cota_m": 812.50, "zona": "LOB", "comentario": "extremo izquierdo"},
        {"km": None, "station_m": -5.0, "x": None, "y": None, "cota_m": 811.00, "zona": "Channel", "comentario": "banco izquierdo"},
        {"km": None, "station_m": 0.0, "x": None, "y": None, "cota_m": 810.20, "zona": "Channel", "comentario": "talweg"},
        {"km": None, "station_m": 5.0, "x": None, "y": None, "cota_m": 811.10, "zona": "Channel", "comentario": "banco derecho"},
        {"km": None, "station_m": 20.0, "x": None, "y": None, "cota_m": 812.70, "zona": "ROB", "comentario": "extremo derecho"},
    ])
    bio = io.BytesIO()
    with pd.ExcelWriter(bio, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="puntos_seccion")
        pd.DataFrame([
            {"campo": "km", "descripcion": "Opcional. Si se informa, permite definir varias secciones por km."},
            {"campo": "station_m", "descripcion": "Obligatorio. Estación transversal, creciente de izquierda a derecha mirando aguas abajo."},
            {"campo": "x", "descripcion": "Opcional/recomendado. Coordenada UTM WGS84 X del punto."},
            {"campo": "y", "descripcion": "Opcional/recomendado. Coordenada UTM WGS84 Y del punto."},
            {"campo": "cota_m", "descripcion": "Obligatorio. Cota del punto."},
        ]).to_excel(writer, index=False, sheet_name="diccionario")
    return bio.getvalue()


def _normalize_section_points_df(df: pd.DataFrame) -> pd.DataFrame:
    cols = {str(c).strip().lower(): c for c in df.columns}
    aliases = {
        "km": ["km", "pk", "progresiva", "progresiva_km", "river_station"],
        "station_m": ["station_m", "station", "estacion", "estación", "distancia_transversal", "offset", "station [m]"],
        "cota_m": ["cota_m", "cota", "elev", "elevacion", "elevación", "z", "elevation", "cota [m]"],
        "x": ["x", "utm_x", "este", "easting", "coord_x"],
        "y": ["y", "utm_y", "norte", "northing", "coord_y"],
    }
    out = pd.DataFrame()
    for key, opts in aliases.items():
        ...
        if src is not None:
            ...
            out[key] = np.nan
    if out["station_m"].isna().all() or out["cota_m"].isna().all():
        raise RuntimeError("La tabla debe contener al menos station_m y cota_m.")
    for c in ["km", "station_m", "cota_m", "x", "y"]:
        out[c] = pd.to_numeric(out[c], errors="coerce")
    out = out.dropna(subset=["station_m", "cota_m"]).copy()
    if len(out) < 2:
        raise RuntimeError("Se requieren al menos dos puntos station-cota para construir una sección.")
    return out


def read_section_points_upload(uploaded_file) -> pd.DataFrame:
    if uploaded_file is None:
        raise RuntimeError("Debe cargar un Excel o CSV con puntos de sección.")
    name = uploaded_file.name.lower()
    data = uploaded_bytes(uploaded_file)
    if name.endswith((".xlsx", ".xls")):
    ...
    return _normalize_section_points_df(df)


def _section_from_station_points(axis_useful: LineString, km: float, pts_df: pd.DataFrame,
                                 source: str, plan: str = "Modificado") -> CrossSection:
    d = min(max(float(km) * 1000.0, 0.0), float(axis_useful.length))
    pts = pts_df.dropna(subset=["station_m", "cota_m"]).sort_values("station_m").copy()
    # Eliminar estaciones duplicadas conservando la primera, porque HEC-RAS requiere station creciente.
    pts = pts.drop_duplicates(subset=["station_m"], keep="first")
    stations = [float(v) for v in pts["station_m"].tolist()]
    elevations = [float(v) for v in pts["cota_m"].tolist()]
    center = axis_useful.interpolate(d)
    has_xy = pts[["x", "y"]].notna().all(axis=1).sum() >= 2
    if has_xy:
        first = pts.iloc[0]
        last = pts.iloc[-1]
        left_x, left_y = float(first["x"]), float(first["y"])
        right_x, right_y = float(last["x"]), float(last["y"])
    else:
        max_abs = max(abs(min(stations)), abs(max(stations)), (max(stations) - min(stations)) / 2.0, 1.0)
        sec_line = cross_section_line(axis_useful, d, max_abs)
        left_x, left_y = sec_line.coords[0]
        right_x, right_y = sec_line.coords[-1]
    sec = CrossSection(
        km=float(km),
        center_x=float(center.x), center_y=float(center.y),
        left_x=float(left_x), left_y=float(left_y),
        right_x=float(right_x), right_y=float(right_y),
        stations=stations,
        elevations=elevations,
        source=source,
        status="corregida",
        notes="Sección insertada desde puntos station-cota-coordenadas. No corresponde a DEM ni levantamiento automático.",
        origin_type="modificada",
        section_type="insertada_por_puntos",
        plan=plan,
        topo_source="Excel/CSV puntos",
    )
    # Bancos/talweg se estiman, luego pueden ajustarse en editor hidráulico.
    _append_change(sec, "insertar_seccion_por_puntos", "geometria", "sin sección", {"km": km, "n_puntos": len(stations)})
    return qa_section(sec, axis_useful=axis_useful)


def insert_sections_from_points_between_km(sections: List[CrossSection], axis_useful: LineString,
                                           points_df: pd.DataFrame,
                                           km_from: float, km_to: float, spacing_m: float,
                                           replace_existing: bool = True,
                                           plan: str = "Modificado") -> Tuple[List[CrossSection], int, int]:
    """Inserta secciones definidas por puntos station-cota-coordenadas.

    Si la tabla trae columna km con valores, se agrupa por km. Si no trae km, replica la misma
    geometría entre km_from y km_to con separación spacing_m.
    """
    a, b = sorted([float(km_from), float(km_to)])
    if points_df["km"].notna().any():
        km_values = sorted(float(k) for k in points_df["km"].dropna().unique() if a <= float(k) <= b)
        grouped = [(k, points_df[np.isclose(points_df["km"].astype(float), float(k), equal_nan=False)].copy()) for k in km_values]
    else:
        step_km = max(float(spacing_m) / 1000.0, 0.001)
        km_values = list(np.arange(a, b + step_km * 0.5, step_km))
        if km_values and km_values[-1] > b + 1e-6:
            km_values[-1] = b
        if not km_values or abs(km_values[-1] - b) > 1e-6:
            km_values.append(b)
        grouped = [(float(k), points_df.copy()) for k in km_values]
    new_secs = [_section_from_station_points(axis_useful, k, g, "Sección insertada por puntos", plan=plan) for k, g in grouped]
    if replace_existing:
        kept = [s for s in sections if not (a <= float(s.km) <= b)]
    else:
        kept = list(sections)
    merged = sorted(kept + new_secs, key=lambda s: float(s.km))
    return apply_sections_qa(merged, axis_useful=axis_useful), len(new_secs), (len(sections) - len(kept))
...
(data.sections || []).forEach(s => {
  ...
  L.marker(s.pts[1], {
    icon: L.divIcon({
      className: 'km-label',
      html: '<div style="background: rgba(0,0,0,0.65); color: #fff; border: 1px solid rgba(255,255,255,0.35); border-radius: 4px; padding: 1px 4px; font: 11px Arial; white-space: nowrap;">km ' + s.km.toFixed(3) + '</div>',
      iconSize: [64, 16],
      iconAnchor: [32, -4]
    })
  }).addTo(map);
...
legend.onAdd = function() {
  ...
  div.innerHTML = '<b>Referencia satelital</b><br><span style="color:#ffd400">━━</span> Eje útil<br><span style="color:#00e5ff">━━</span> Sección aceptada/revisar<br><span style="color:#00ff66">━━</span> Sección corregida<br><span style="color:#ff3333">━━</span> Sección rechazada<br><span style="color:#ffffff">▣</span> Etiqueta km por sección';
  return div;
};
legend.addTo(map);
...
def render_results(result: Dict[str, Any]):
    ...
    sections = apply_sections_qa(result.get("sections", []), result.get("axis_useful"))
    ...
    with tab2:
        ...
        if not sections:
            ...
            df_secs = pd.DataFrame([{
                ...
                "ancho_total_m": s.total_width_m,
                "ancho_canal_m": s.main_channel_width_m,
                "talweg_station": s.talweg_station,
                "banco_izq": s.left_bank_station,
                "banco_der": s.right_bank_station,
                "n_channel": s.manning_channel,
                ...
                "qa_warnings": "; ".join(s.qa_warnings or []),
                "notes": s.notes,
            } for i, s in enumerate(sections)])
            st.dataframe(df_secs, use_container_width=True, height=450)
            with st.expander("Control adaptativo de espaciamiento", expanded=False):
                st.caption("Recomendación geométrica preliminar. La prueba de convergencia hidráulica completa se ejecutará cuando este módulo se integre al cálculo hidráulico de HidroSed.")
                spacing_df = adaptive_spacing_recommendations(sections, axis_useful)
                st.dataframe(spacing_df, use_container_width=True, height=300)
    ...
    with tab3:
        ...
        if not sections:
            ...
            with col_a:
                fig_int = plot_single_section_interactive(sec)
            ...
            with col_b:
                ...
                with st.expander("Propiedades hidráulicas y puntos maestros", expanded=True):
                    st_min = float(min(sec.stations)) if sec.stations else 0.0
                    st_max = float(max(sec.stations)) if sec.stations else 1.0
                    hp1, hp2, hp3 = st.columns(3)
                    new_lbank = hp1.number_input("Banco izquierdo [station m]", value=float(sec.left_bank_station if sec.left_bank_station is not None else st_min), step=0.1, key=f"lbank_{current_idx}")
                    new_talweg = hp2.number_input("Talweg [station m]", value=float(sec.talweg_station if sec.talweg_station is not None else (st_min + st_max) / 2.0), step=0.1, key=f"talweg_{current_idx}")
                    new_rbank = hp3.number_input("Banco derecho [station m]", value=float(sec.right_bank_station if sec.right_bank_station is not None else st_max), step=0.1, key=f"rbank_{current_idx}")
                    hn1, hn2, hn3 = st.columns(3)
                    new_n_lob = hn1.number_input("Manning LOB", min_value=0.005, max_value=0.300, value=float(sec.manning_lob), step=0.001, format="%.3f", key=f"n_lob_{current_idx}")
                    new_n_ch = hn2.number_input("Manning Channel", min_value=0.005, max_value=0.300, value=float(sec.manning_channel), step=0.001, format="%.3f", key=f"n_ch_{current_idx}")
                    new_n_rob = hn3.number_input("Manning ROB", min_value=0.005, max_value=0.300, value=float(sec.manning_rob), step=0.001, format="%.3f", key=f"n_rob_{current_idx}")
                    hc1, hc2 = st.columns(2)
                    new_contr = hc1.number_input("Coef. contracción", min_value=0.0, max_value=1.0, value=float(sec.contraction_coeff), step=0.01, format="%.2f", key=f"contr_{current_idx}")
                    new_exp = hc2.number_input("Coef. expansión", min_value=0.0, max_value=1.0, value=float(sec.expansion_coeff), step=0.01, format="%.2f", key=f"exp_{current_idx}")
                    hi1, hi2, hi3 = st.columns(3)
                    new_ineff = hi1.text_input("Áreas de flujo ineficaz", value=sec.ineffective_flow or "", key=f"ineff_{current_idx}")
                    new_obs = hi2.text_input("Obstrucciones", value=sec.obstructions or "", key=f"obs_{current_idx}")
                    new_lev = hi3.text_input("Diques/límites ribera", value=sec.levees_or_limits or "", key=f"lev_{current_idx}")
                ...
                if c1.button("Aplicar edición", key=f"apply_{current_idx}"):
                    try:
                        sec.left_bank_station = float(new_lbank)
                        sec.right_bank_station = float(new_rbank)
                        sec.talweg_station = float(new_talweg)
                        sec.manning_lob = float(new_n_lob)
                        sec.manning_channel = float(new_n_ch)
                        sec.manning_rob = float(new_n_rob)
                        sec.contraction_coeff = float(new_contr)
                        sec.expansion_coeff = float(new_exp)
                        sec.ineffective_flow = str(new_ineff)
                        sec.obstructions = str(new_obs)
                        sec.levees_or_limits = str(new_lev)
                ...
                if c4.button("Rechazar", key=f"reject_{current_idx}"):
                    ...
                    st.session_state["eje_result"] = result
                    st.rerun()
                if c5.button("Eliminar", key=f"delete_{current_idx}"):
                    deleted_km = sections[current_idx].km
                    del sections[current_idx]
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.success(f"Se eliminó la sección km {deleted_km:.3f}.")
                    st.rerun()

                st.caption(f"QA: {sec.qa_status} · " + ("; ".join(sec.qa_warnings or []) if sec.qa_warnings else "sin advertencias"))
                e1, e2, e3, e4 = st.columns(4)
                if e1.button("Restaurar original", key=f"restore_{current_idx}", disabled=not bool(sec.original_stations)):
                    sections[current_idx] = restore_original_section(sec)
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.rerun()
                if e2.button("Duplicar sección", key=f"dup_{current_idx}"):
                    sections.append(duplicate_section(sec, delta_km=0.001))
                    sections = sorted(sections, key=lambda s: float(s.km))
                    result["sections"] = apply_sections_qa(sections, axis_useful)
                    st.session_state["eje_result"] = result
                    st.rerun()
                if e3.button("Copiar prop. anterior", key=f"copy_prev_{current_idx}", disabled=(current_idx == 0)):
                    sections[current_idx] = copy_hydraulic_properties(sec, sections[current_idx-1], "copiar_desde_anterior")
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.rerun()
                if e4.button("Copiar prop. siguiente", key=f"copy_next_{current_idx}", disabled=(current_idx >= len(sections)-1)):
                    sections[current_idx] = copy_hydraulic_properties(sec, sections[current_idx+1], "copiar_desde_siguiente")
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.rerun()
...
"""
            ...
            with st.expander("Insertar secciones por puntos: km + coordenadas + cota + station", expanded=False):
                st.markdown(
                    """
Esta herramienta inserta una o varias secciones definidas por puntos. Debes indicar el tramo
**desde km X hasta km Y** y cargar una tabla con, al menos, `station_m` y `cota_m`.
También puede incluir `x` e `y` UTM WGS84 por cada punto. Si la tabla trae columna `km`,
la app agrupa los puntos por km; si no trae km, replica la misma geometría con la separación indicada.
"""
                )
                st.download_button(
                    "Descargar plantilla puntos de sección",
                    section_points_template_excel(),
                    file_name="plantilla_insertar_seccion_puntos.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="template_insert_points",
                )
                ip1, ip2, ip3, ip4 = st.columns(4)
                km_from_ip = ip1.number_input("Insertar desde km", min_value=min_km, max_value=max_km, value=km_from, step=0.001, format="%.3f", key="ip_km_from")
                km_to_ip = ip2.number_input("Insertar hasta km", min_value=min_km, max_value=max_km, value=km_to, step=0.001, format="%.3f", key="ip_km_to")
                sep_ip = ip3.number_input("Separación longitudinal [m]", min_value=1.0, max_value=5000.0, value=float(result.get("section_spacing", 100.0)), step=5.0, key="ip_sep")
                replace_ip = ip4.checkbox("Reemplazar secciones existentes del tramo", value=False, key="ip_replace")
                plan_ip = st.text_input("Plan / alternativa", value="Modificado", key="ip_plan")

                ip_mode_tab1, ip_mode_tab2 = st.tabs(["Ingreso directo en la app", "Cargar Excel/CSV"])
                with ip_mode_tab1:
                    st.caption("Ingresa directamente los puntos de la sección. `station_m` y `cota_m` son obligatorios. `km`, `x` e `y` son opcionales/recomendados.")
                    default_direct = pd.DataFrame([
                        {"km": np.nan, "station_m": -20.0, "x": np.nan, "y": np.nan, "cota_m": 812.50},
                        {"km": np.nan, "station_m": -5.0, "x": np.nan, "y": np.nan, "cota_m": 811.00},
                        {"km": np.nan, "station_m": 0.0, "x": np.nan, "y": np.nan, "cota_m": 810.20},
                        {"km": np.nan, "station_m": 5.0, "x": np.nan, "y": np.nan, "cota_m": 811.10},
                        {"km": np.nan, "station_m": 20.0, "x": np.nan, "y": np.nan, "cota_m": 812.70},
                    ])
                    direct_points_df = st.data_editor(
                        default_direct,
                        num_rows="dynamic",
                        use_container_width=True,
                        key="insert_points_direct_editor",
                        column_config={
                            "km": st.column_config.NumberColumn("km", help="Opcional. Si se informa, agrupa puntos por progresiva."),
                            "station_m": st.column_config.NumberColumn("station_m", required=True, help="Estación transversal creciente de izquierda a derecha mirando aguas abajo."),
                            "x": st.column_config.NumberColumn("x UTM", help="Opcional/recomendado. Coordenada X del punto."),
                            "y": st.column_config.NumberColumn("y UTM", help="Opcional/recomendado. Coordenada Y del punto."),
                            "cota_m": st.column_config.NumberColumn("cota_m", required=True, help="Cota del punto."),
                        },
                    )
                    col_dir1, col_dir2 = st.columns([1, 2])
                    if col_dir1.button("Insertar desde tabla directa", key="insert_points_direct_apply"):
                        try:
                            points_df = _normalize_section_points_df(pd.DataFrame(direct_points_df))
                            sections, nins, nrep = insert_sections_from_points_between_km(
                                sections, axis_useful, points_df, km_from_ip, km_to_ip, sep_ip,
                                replace_existing=replace_ip, plan=plan_ip,
                            )
                            result["sections"] = sections
                            st.session_state["eje_result"] = result
                            msg = f"Se insertaron {nins} secciones desde la tabla directa."
                            if replace_ip:
                                msg += f" Se reemplazaron {nrep} secciones previas del tramo."
                            st.success(msg)
                            st.rerun()
                        except Exception as exc:
                            st.error(str(exc))
                    col_dir2.info("Si no completas la columna km, la misma sección se replicará entre km inicial y km final usando la separación indicada.")

                with ip_mode_tab2:
                    ip_file = st.file_uploader(
                        "Archivo Excel/CSV con puntos de sección",
                        type=["xlsx", "xls", "csv"],
                        key="insert_points_file",
                        help="Columnas esperadas: km, station_m, x, y, cota_m. km es opcional si se replica la sección por separación.",
                    )
                    if ip_file is not None:
                        try:
                            preview_ip = read_section_points_upload(ip_file)
                            st.caption("Vista previa normalizada de puntos a insertar")
                            st.dataframe(preview_ip.head(20), use_container_width=True, height=220)
                        except Exception as exc:
                            st.error(f"No se pudo leer la tabla de puntos: {exc}")
                    if st.button("Insertar secciones desde archivo", key="insert_points_apply"):
                        try:
                            points_df = read_section_points_upload(ip_file)
                            sections, nins, nrep = insert_sections_from_points_between_km(
                                sections, axis_useful, points_df, km_from_ip, km_to_ip, sep_ip,
                                replace_existing=replace_ip, plan=plan_ip,
                            )
                            result["sections"] = sections
                            st.session_state["eje_result"] = result
                            msg = f"Se insertaron {nins} secciones desde puntos."
                            if replace_ip:
                                msg += f" Se reemplazaron {nrep} secciones previas del tramo."
                            st.success(msg)
                            st.rerun()
            ...
            with st.expander("Eliminar secciones por tramo km", expanded=False):
                d1, d2, d3 = st.columns(3)
                del_km_from = float(d1.number_input("Eliminar desde km", min_value=min_km, max_value=max_km, value=km_from, step=0.001, format="%.3f", key="del_km_from"))
                del_km_to = float(d2.number_input("Eliminar hasta km", min_value=min_km, max_value=max_km, value=km_to, step=0.001, format="%.3f", key="del_km_to"))
                keep_min = bool(d3.checkbox("Proteger primera/última sección", value=True, key="del_keep_end"))
                if st.button("Eliminar secciones del tramo", key="delete_range_sections"):
                    k0, k1 = sorted([del_km_from, del_km_to])
                    new_sections = []
                    removed = 0
                    for idx, secx in enumerate(sections):
                        protect = keep_min and idx in (0, len(sections)-1)
                        if (k0 <= float(secx.km) <= k1) and not protect:
                            removed += 1
                        else:
                            new_sections.append(secx)
                    sections = new_sections
                    result["sections"] = sections
                    st.session_state["eje_result"] = result
                    st.success(f"Se eliminaron {removed} secciones en el tramo km {k0:.3f} – {k1:.3f}.")
                    st.rerun()
    ...
    with tab4:
        ...
        st.markdown(
            """
La vista 3D ahora puede **unificar longitudinalmente las secciones** como geometría 1D tipo HEC-RAS:
las superficies conectan puntos homologables de sobrebanco izquierdo, canal principal y sobrebanco derecho.
Esto permite revisar continuidad hidráulica antes de aplicar ecuaciones de energía, Saint-Venant o análisis de flujo permanente/no permanente.
"""
        )
        cmesh1, cmesh2 = st.columns(2)
        show_connected_mesh = cmesh1.checkbox("Mostrar malla longitudinal conectada tipo HEC-RAS", value=True, key="show_hecras_mesh")
        include_rejected_mesh = cmesh2.checkbox("Incluir secciones rechazadas en la malla", value=False, key="include_rejected_mesh")
        fig3d = plot_sections_3d_interactive(axis_useful, profile_df, sections, connected_mesh=show_connected_mesh, include_rejected_mesh=include_rejected_mesh)
        ...
        with st.expander("Tabla de conectividad longitudinal HEC-RAS", expanded=False):
            conn_df = hecras_connection_table(sections)
            if conn_df.empty:
                st.info("No hay suficientes secciones válidas para calcular conectividad longitudinal.")
            else:
                st.dataframe(conn_df, use_container_width=True, height=320)
        st.markdown(
            """
**Uso recomendado de revisión:**
1. Recorre la malla longitudinal 3D y verifica que las zonas LOB, Channel y ROB no se crucen ni se inviertan.
2. Revisa bancos y talweg en la pestaña **Ventana sección por km**.
3. Corrige o elimina secciones que produzcan torsión longitudinal.
...
"""
    ...
    with tab6:
        ...
        st.download_button("Descargar KMZ eje + secciones", kmz_bytes, file_name="hidrosed_eje_cauce_secciones_v12_hecras_connect.kmz", mime="application/vnd.google-earth.kmz")
        ...
        summary = {
            "modulo": "HidroSed Eje Cauce y Secciones v12 conectividad longitudinal HEC-RAS",
            ...
            "secciones_por_QA": pd.Series([s.qa_status for s in sections]).value_counts().to_dict() if sections else {},
            "advertencias_QA_total": sum(len(s.qa_warnings or []) for s in sections),
            "bitacora_cambios": sections_change_log_df(sections).to_dict(orient="records"),
            "conectividad_longitudinal": hecras_connection_table(sections).to_dict(orient="records"),
            "fuente_dem": result.get("dem_source"),
            "descarga_opentopography": result.get("dem_download_info"),
        }
        st.download_button("Descargar JSON técnico actualizado", json.dumps(summary, indent=2, ensure_ascii=False).encode("utf-8"), file_name="resumen_tecnico_eje_secciones_v12.json", mime="application/json")


# --------------------------------------------------------------------------------------
# App UI
# --------------------------------------------------------------------------------------
st.title("🌊 HidroSed · Módulo Eje Cauce y Secciones v12")
st.caption("Eje útil obligatorio · QA hidráulico · modelo geométrico común · conectividad longitudinal tipo HEC-RAS · perfil 3D")
...
- modelo geométrico común por sección: bancos, talweg, Manning LOB/Channel/ROB, contracción/expansión, QA y bitácora;
- interpolación avanzada entre secciones ancla con puntos maestros;
- control adaptativo preliminar del espaciamiento.
"""
    )
